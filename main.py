...
class AuctioConnectionManager:
    def __init__(self) -> None:
        self.auction_connections: Dict[any, list] = {}

    async def connect(self, websocket: WebSocket, auction_id):
        await websocket.accept()
        item_found = session.get(ActionItem, auction_id)
        print(item_found)
        if (
            item_found.ends and item_found.ends < datetime.datetime.now()
        ) or item_found.completed:
            item_found.completed = True
            await self.send_personal_message(
                "The auction is already finished", websocket
            )
            await self.send_personal_message(
                f"{item_found.item_name} was sold for {item_found.bid}"
                f" to a bidder #{item_found.bidder_id}!",
                websocket,
            )
            await self.send_personal_message(
                message="", websocket=websocket, json_data={"completed": True}
            )
        ...
        if cur_bid:
            await self.send_personal_message(
                "The auction has already started", websocket, cur_bid
            )
    ...
    async def send_personal_message(
        self, message: str, websocket: WebSocket, cur_price=None, json_data=None
    ):
...
manager = AuctioConnectionManager()
...
async def auctions():
    ...
    auction_items = [i["AuctionItems"] for i in auction_items]
    return auction_items


@app.websocket("/auction/{id}/ws/{participant_id}")
async def auction(websocket: WebSocket, id: int, participant_id: id):
    await manager.connect(websocket, id)
    try:
        while True:
            data = await websocket.receive_json()
            item = session.get(AuctionItem, id)
            step = item.price_step or 0
            current_bid = item.bid or 0
            min_new_bid = (
                current_bid + step if current_bid != item.min_price else current_bid
            )
            new_bid = data.get("bid")
            if not new_bid:
                print(1)
                continue
            if participant_id == item.participant or not new_bid > current_bid:
                print(2)
                continue
            if item.min_price <= new_bid >= min_new_bid:
                ...
                await manager.broadcast(
                    f"Participant {participant_id} has bid {item.bid}!",
                    auction_id=id,
                    new_price=item.id,
                    ends=item.ends,
                )
